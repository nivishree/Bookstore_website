...
public class BookDaoJdbc implements BookDao {
    ...
    public List<Book> findRandomByCategoryId(long categoryId, int limit) {
        List<Book> suggested_books = new ArrayList<>();
        try (Connection connection = JdbcUtils.getConnection();
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                while(resultSet.next()){
                    suggested_books.add(readBook((resultSet)));
                }
            }
        } catch (SQLException e) {
            throw new BookstoreQueryDbException("Encountered a problem finding books by category ID  " + categoryId, e);
    ...
    @Override
    public List<Book> findRandomByCategoryName(Category result, int limit) {
        List<Book> suggestedBooksByName = new ArrayList<>();
        try (Connection connection = JdbcUtils.getConnection();
             PreparedStatement statement = connection.prepareStatement(FIND_RANDOM_BY_CATEGORY_ID_SQL)) {
            statement.setLong(1, result.getCategoryId());
            statement.setLong(2, limit);
            try (ResultSet resultSet = statement.executeQuery()) {
                while(resultSet.next()){
                    suggestedBooksByName.add(readBook((resultSet)));
                }
            }
        } catch (SQLException e) {
            throw new BookstoreQueryDbException("Encountered a problem finding books by category Name  " + result.getName(), e);
        }

        return suggestedBooksByName;
    }
